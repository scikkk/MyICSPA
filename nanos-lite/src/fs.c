...
size_t fs_read(int fd, void *buf, size_t len){
	assert(fd > 2);
	if((fd == FD_EVENTS || fd >= FD_NORMAL) && (file_table[fd].open_offset >= file_table[fd].size || len == 0))
		return 0;
	if((fd == FD_EVENTS || fd >= FD_NORMAL) && file_table[fd].open_offset + len > file_table[fd].size)
